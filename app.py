...
if user_menu == "Overall Analysis":
    ...
    fig = px.line(events_overtime, x="editions", y="Event")
    st.title("Events over the years")
    st.plotly_chart(fig)

    athletes_overtime = helper.data_overtime(df, "Name")
    fig = px.line(athletes_overtime, x="editions", y="Name")
...
if user_menu == "Country Wise Analysis":
    ...
    st.title(selected_country + " Excels in folowing sports")
...
if user_menu == "Athlete Wise Analysis":
    ...
    fig.update_layout(autosize=False, width=1000, height=600)
    st.title("Gold Distribution Age wrt Sports ")
    st.plotly_chart(fig)

    sport_list = df["Sport"].unique().tolist()
    sport_list.sort()
    sport_list.insert(0,"Overall")
    wants_drop = st.selectbox("Want to drop Non winners ? ",[True,False])

    st.title("Height vs Weight")
    selected_sport = st.selectbox("Select a sport",sport_list)

    temp_df = helper.weight_v_height(df,selected_sport,wants_drop)
    fig ,ax = plt.subplots()
